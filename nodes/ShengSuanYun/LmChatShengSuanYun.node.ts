...
export class LmChatShengSuanYun implements INodeType {
	description: INodeTypeDescription = {
		...
		description: 'Chat with the ShengSuanYun API',
		defaults: {
			name: 'ShengSuanYun Chat Model',
		},
		inputs: [],
		outputs: [NodeConnectionTypes.AiLanguageModel],
		credentials: [{ name: 'shengSyanYunApi', required: true }],
		requestDefaults: {
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'https://docs.n8n.io/integrations/builtin/cluster-nodes/sub-nodes/n8n-nodes-langchain.lmchatshengsuanyun/',
					},
				],
			},
		},
		...
		properties: [
			{
				...
				name: 'model',
				type: 'options',
				description:'The model which will generate the completion. <a href="https://router.shengsuanyun.com/api/v1/models">Learn more</a>.',
				typeOptions: {
					loadOptions: {
						routing: {
							request: {
	...
	async supplyData(this: ISupplyDataFunctions, itemIndex: number): Promise<SupplyData> {
		const credentials = await this.getCredentials<OpenAICompatibleCredential>('shengSyanYunApi');
